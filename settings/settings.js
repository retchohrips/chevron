...
const template = {
  general: {
    ...
    */
    ...
    searchEngine: new types.List("brave", searchEngines),
    ...
    tabTitle: new types.Input("New Tab"),
    // hidden
  ...
  chevron: {
    thickness: new types.Range(15, { min: 1, max: 50 }, { format: "{@}px" }),
    size: new types.Range(20, undefined, { format: "{@}%" }),
    quickLook: {
      marquee: new types.Switch(true),
      showMacrosLabel: new types.Switch(false),
      // hidden
      topCurvature: new types.Range(0.3, { min: 0, max: 1, step: 0.1 }),
      // hidden
      bottomCurvature: new types.Range(0.4, { min: 0, max: 1, step: 0.1 }),
    },
  },
  query: {
    ...
    suggestions: {
      ...
      autocompleteLimit: new types.Range(10, { min: 0, max: 50 }),
      historyLimit: new types.Range(5, { min: 0, max: 50 }),
    },
    AI: {
      enabled: new types.Switch(false),
      apiKey: new types.Input("", "Enter your openai api key"),
      temperature: new types.Range(0.4, { min: 0, max: 1, step: 0.05 }),
      language: new types.Input(""),
    },
  },
  menu: {
    rows: new types.Range(2, { min: 1, max: 20 }),
    columns: new types.Range(4, { min: 1, max: 20 }),
    gap: new types.Range(10, { min: 0, max: 50 }, { format: "{@}px" }),
...
class Settings {
  ...
  get hidden() {
    ...
    for (const theme of Object.keys(this.template.appearance.themes)) {
      // light theme
      hiddenColors.forEach((hC) =>
        result.push("appearance.themes." + theme + ".light." + hC),
      );
      // dark theme
      hiddenColors.forEach((hC) =>
        result.push("appearance.themes." + theme + ".dark." + hC),
      );
    }
